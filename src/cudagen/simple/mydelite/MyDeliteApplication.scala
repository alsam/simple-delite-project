...
import my.delite.framework.codegen.cpp.MyTargetCpp
...
import ppl.delite.framework.codegen.restage.TargetRestage
//import ppl.delite.framework.codegen.Target
import my.delite.framework.codegen.MyTarget
import ppl.delite.framework.ops.DeliteOpsExp
//import ppl.delite.framework.transform.{DeliteTransform, DistributedArrayTransformer}
import my.delite.framework.transform.{MyDeliteTransform}
import ppl.delite.framework.transform.{DistributedArrayTransformer}
import ppl.delite.framework.{Config}


trait MyDeliteApplication extends DeliteOpsExp with ScalaCompile with MyDeliteTransform with DeliteAllOverridesExp {  
  type MyDeliteApplicationTarget = MyTarget{val IR: MyDeliteApplication.this.type}

  /*
   * code generators
   */
  def getCodeGenPkg(t: MyDeliteApplicationTarget) : GenericFatCodegen{val IR: MyDeliteApplication.this.type}

  lazy val scalaTarget = new MyTargetScala{val IR: MyDeliteApplication.this.type = MyDeliteApplication.this}
  lazy val cudaTarget = new MyTargetCuda{val IR: MyDeliteApplication.this.type = MyDeliteApplication.this}
  lazy val cppTarget = new MyTargetCpp{val IR: MyDeliteApplication.this.type = MyDeliteApplication.this}
  ...
  def targets = {
    var target = List[MyDeliteApplicationTarget](scalaTarget)
  ...
  lazy val generators: List[GenericFatCodegen{ val IR: MyDeliteApplication.this.type }] = targets.reverse.map(getCodeGenPkg(_))


  // TODO: refactor, this is from ScalaCompile trait
  lazy val codegen: ScalaCodegen { val IR: MyDeliteApplication.this.type } = 
    getCodeGenPkg(scalaTarget).asInstanceOf[ScalaCodegen { val IR: MyDeliteApplication.this.type }]

  // generators created by getCodeGenPkg will use the 'current' scope of the deliteGenerator as global scope
  lazy val deliteGenerator = new MyDeliteCodeGenPkg { val IR : MyDeliteApplication.this.type = MyDeliteApplication.this;
                                               val generators = MyDeliteApplication.this.generators; }
  ...
  */
  ...
  def emitRegisteredSource(gen: GenericFatCodegen{val IR: MyDeliteApplication.this.type}, stream: PrintWriter): List[(Sym[Any], Any)] = arity match {
