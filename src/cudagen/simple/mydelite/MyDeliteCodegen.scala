package my.delite.framework.codegen.delite
 ...
 */
trait MyDeliteCodegen extends GenericFatCodegen with BaseGenStaticData with ppl.delite.framework.codegen.Utils {
  val IR: DeliteOpsExp
  import IR._

  // these are the target-specific kernel generators (e.g. scala, cuda, etc.)
  type Generator = GenericFatCodegen{val IR: MyDeliteCodegen.this.IR.type}
  ...
  var transformers: List[WorklistTransformer{val IR: MyDeliteCodegen.this.IR.type}] = Nil
...
trait MyDeliteCodeGenPkg extends DeliteGenTaskGraph
